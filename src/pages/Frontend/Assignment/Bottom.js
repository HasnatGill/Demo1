import React from 'react'

function Bottom() {
  return (
    <div className="BottomContent">
      <div className="row">
        <div className="col mt-3 ms-4">
          <p><b>US Headquarters:</b>  to Friday,24 Hours</p>
          <p>Sat/Sun/Holiday,EST 09:00-22:00 <b>call:</b> +1 1 212-828-0100</p>
          <p>Closed on Saturdays,Sundays, and Public Holidays <b>Call:</b> 070-8095-1520 </p>
          <p><b>New York Lounge hours</b></p>
          <p>Monday-Saturday 9:00-18:00</p>
          <p>Closed:Sundays,Memorial,Independent,Thanksgiving,Christmas,New <br /> Year in store purchase and consultation with coordinator 14:00-17:00 (Cash payment only)</p>
        </div>
      </div>
    </div>
  )
}

export default Bottom
